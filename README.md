...
**1. 시험 관리 기능**
    ...
    - 시험 목록 / 상세 조회 / 결과 조회 페이지 구현

    - 시험 응시 UI 개발 / 객관식 문항 선택, 타이머 기능, 제출 처리

    - 시험 응시 후 자동 채점 로직 (객관식) 구현

    - 시험 등록, 수정, 삭제, 조회 기능 API 구현 (교수자)
      
    - 시험 응시 데이터 처리 및 제출 기능 API 구현 (학부생)
...
**3. 메시지 기능**
    ...
    - 메시지 관리 UI 구현/ 메시지 목록, 상세 보기, 읽음 여부 표시  
    
    - 메시지 송수신, 목록 및 상세 조회, 주소록 조회, 읽음 여부 표시, 삭제 기능 API 구현
      
    - 사용자별 메시지 목록 시간순 정렬 처리
