...
public class AttendanceController {
 ...
 // 지난 출석 데이터 조회 엔드포인트
    ...
    public ResponseEntity<List<Attendance>> getMonthlyAttendanceForClass(
            @PathVariable("classId") int classId,
            @RequestParam("month") String month) {
        List<Attendance> attendanceList = attendanceService.getMonthlyAttendanceForClass(classId, month);
        return ResponseEntity.ok(attendanceList);
    }
